...
router.post('/api/processes/:id/monitoring', requireAuth, validateProcessId, async (req, res) => {
  try {
    if (!consumeCsrfToken(req, req.session)) {
      logger.warn(
        `[SECURITY] CSRF token mismatch for user: ${req.session.username} during monitoring toggle for process ${req.params.id} from identity: ${getClientIdentity(req)}`,
      );
      return res.status(403).json({ error: 'CSRF token mismatch' });
    }
    const pm2Name = (await resolvePm2Name(req.params.id).catch(() => null)) ?? req.params.id;
