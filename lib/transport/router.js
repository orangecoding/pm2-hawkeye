...
import { validateAppName, validateRepoUrl, resolveDeployPath, runDeploy, resolveConfirmation } from '../service/deployRunner.js';
import {
  createDeployment,
  updateDeployment,
  getDeploymentByName,
...
router.post('/api/deployments', requireAuth, async (req, res) => {
  ...
  if (!validateAppName(appName)) {
    return res.status(400).json({ error: 'Invalid app name. Use alphanumeric characters, dashes, and underscores (max 64 chars).' });
  }
  if (!validateRepoUrl(repoUrl)) {
    return res.status(400).json({ error: 'Invalid repo URL. Accepted: HTTPS GitHub/GitLab URLs or SSH git URLs (e.g. git@github.com:user/repo.git).' });
  }
  ...
  if (existing) {
    return res.status(409).json({ error: `A deployment named "${appName}" already exists. Use the redeploy endpoint to update it.` });
  }
...
router.post('/api/deployments/:deploymentId/confirm', requireAuth, validateDeploymentId, (req, res) => {
  ...
  logger.info(`[DEPLOY] Confirmation ${confirmed ? 'accepted' : 'rejected'} for deployment "${req.params.deploymentId}"`);
  res.json({ ok: true });
});
