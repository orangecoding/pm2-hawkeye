...
import pm2 from 'pm2';
import { groupLogLines } from './logBackfill.js';
...
export async function startLogBus() {
  ...
  try {
    ...
    function handlePacket(packet) {
      ...
      // Persist each packet as timestamped primary lines with any stack-trace
      // continuations attached, matching historical backfill behavior.
      try {
        const row = getByPm2Name(appName);
        if (row) {
          for (const group of groupLogLines(
            lines.map((text) => ({ text, source: 'bus' })),
            at,
          )) {
            insertLogEntry(row.id, group);
            evaluateAndDispatch(appName, group.logLevel, JSON.parse(group.log).raw).catch((err) =>
