...
import { insertHostMetric, purgeOldHostMetrics } from '../storage/hostMetricsStorage.js';
import { collectHostMetrics } from './hostMetricsCollector.js';
...
export function startMetricsScheduler() {
  ...
  const sampler = new Cron('*/20 * * * * *', { protect: true }, async () => {
    try {
      ...
      // Sample host-level CPU, RAM, and disk unconditionally - host metrics
      // must be collected regardless of whether any process is monitored.
      try {
        const { cpu, ram, disk } = await collectHostMetrics();
        insertHostMetric(cpu, ram, disk);
      } catch (hostErr) {
        logger.warn(`[METRICS_SCHEDULER] Host metrics sampling error: ${hostErr.message}`);
      }
