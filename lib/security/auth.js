 ...
 *
 * Both layers are keyed by client IP. User-Agent is intentionally excluded
 * because an attacker can rotate that header to bypass a security limit.
 */
...
/** Maximum identities retained per in-memory limiter. */
const MAX_RATE_LIMIT_ENTRIES = 10_000;

/** Insert a limiter entry while keeping attacker-controlled state bounded. */
function setRateLimitEntry(map, identity, value) {
  if (!map.has(identity) && map.size >= MAX_RATE_LIMIT_ENTRIES) {
...
export function checkLoginWindow(identity, now) {
  ...
  if (!entry || entry.windowEndsAt <= now) {
    setRateLimitEntry(loginRateWindow, identity, {
      count: 1,
...
export function registerFailedAttempt(identity, now) {
  ...
  setRateLimitEntry(loginPenaltyState, identity, {
    failures,
    lastFailureAt: now,
...
export function checkUnauthWindow(identity, now) {
  ...
  if (!entry || entry.windowEndsAt <= now) {
    setRateLimitEntry(unauthRateWindow, identity, { count: 1, windowEndsAt: now + config.UNAUTH_WINDOW_MS });
